...
trait HeapProperties(val heapInterface: HeapInterface):
  ...
  val meldingSmallHeaps: (String, Prop) =
    ...
    forAll { (x: Int, y: Int) =>
      ...
      val high = x.max(y)
      val low = x.min(y)
      val heapHigh = insert(high, insert(high, empty))
      val heapLow = insert(low, insert(low, empty))
      val meldedHeap: List[Node] = meld(heapHigh, heapLow)
      ...
      val deleteTwoMinAndFindMin: Boolean = 
        findMin(deleteMin(deleteMin(meldedHeap))) == high
      ...
      val insertMinAndFindMin: Boolean =
        findMin(insert(low, meldedHeap)) == low
  ...
  val meldingHeaps: (String, Prop) =
    def check(meldedHeap: List[Node], heap1: List[Node], heap2: List[Node]): Boolean =
      if isEmpty(meldedHeap) && isEmpty(heap1) && isEmpty(heap2) then 
        true
      else
        val meldedMin = findMin(meldedHeap)
        if isEmpty(heap1) then meldedMin == findMin(heap2) && check(deleteMin(meldedHeap), heap1, deleteMin(heap2)) else
        if isEmpty(heap2) then meldedMin == findMin(heap1) && check(deleteMin(meldedHeap), deleteMin(heap1), heap2) else
          val min1 = findMin(heap1)
          val min2 = findMin(heap2)
          if meldedMin != min1.min(min2) then
            false
          else if meldedMin == min1 then
            check(deleteMin(meldedHeap), deleteMin(heap1), heap2)
          else
            check(deleteMin(meldedHeap), heap1, deleteMin(heap2))
