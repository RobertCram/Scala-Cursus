...
import wikigraph.WikiResult.traverse
...
final class Wikigraph(client: Wikipedia):
  ...
  def namedLinks(of: ArticleId): WikiResult[Set[String]] =
    client
      .linksFrom(of)
      .flatMap(links => traverse(links.toSeq)(articleId => client.nameOfArticle(articleId)).map(_.toSet))
  ...
  def breadthFirstSearch(start: ArticleId, target: ArticleId, maxDepth: Int): WikiResult[Option[Int]] =
    ...
    def iter(visited: Set[ArticleId], q: Queue[(Int, ArticleId)]): WikiResult[Option[Int]] =
      if q.isEmpty then WikiResult.successful(None) else
        val ((depth, articleId), tailQ) = q.dequeue
        if depth == maxDepth then WikiResult.successful(None) else
          client.linksFrom(articleId).flatMap(
            links => if links.contains(target) then WikiResult.successful(Some(depth)) else
              val newVisited = visited ++ links
              val newQ = tailQ.enqueue((links -- visited).map(articleId => depth + 1 -> articleId))
              iter(newVisited, newQ)
          ).fallbackTo(iter(visited, tailQ))
  ...
  def distanceMatrix(titles: List[String], maxDepth: Int = 50): WikiResult[Seq[(String, String, Option[Int])]] =
    ...
    case class Article(id: ArticleId, title: String)

    val articleList = traverse[String, (ArticleId, String)](titles)(title => client.searchId(title).zip(WikiResult.successful(title))).map(seq => seq.map(article => Article(article(0), article(1))))

    for {
      articles <- articleList
      combinations <-  WikiResult.successful(articles.flatMap(article1 => articles.filter(_ != article1).map(article2 => (article1, article2))))
      result <- traverse(combinations)(combination => breadthFirstSearch(combination(0).id, combination(1).id, maxDepth).zip(WikiResult.successful(combination)))
    } yield(result.map(r => (r(1)(0).title, r(1)(1).title, r(0))))

    
    // val combinedArticleList = articleList.flatMap(
    //   articles => WikiResult.successful(
    //     articles.flatMap(article1 => articles.filter(_ != article1).map(article2 => (article1, article2)))
    //   )        
    // )

    // combinedArticleList.flatMap(
    //   combinations => traverse(combinations)(combination => breadthFirstSearch(combination(0).id, combination(1).id, maxDepth).zip(WikiResult.successful(combination)))
    //   .flatMap(result => WikiResult.successful(result.map(r => (r(1)(0).title, r(1)(1).title, r(0)))))
    // )
