...
trait DecoderInstances:
  ...
  given intDecoder: Decoder[Int] =
    Decoder.fromFunction { 
      case Json.Num(bd) => if bd.isValidInt then Some(bd.toInt) else None
      case _ => None
    }
  ...
  given listDecoder[A](using decoder: Decoder[A]): Decoder[List[A]] =
    ...
    def processItem(item: Json, maybePreviouslyDecodedItems: Option[List[A]]): Option[List[A]] =
      maybePreviouslyDecodedItems match
        case Some(as) => decodeAndPrepend(item, as)
        case None => None
    ...
    Decoder.fromFunction {
      case Json.Arr(list) => decodeAllItems(list)
      case _ => None
    }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Json.Obj(map) => map(name).decodeAs[A]
      case _ => None
    }
...
trait ContactsCodecs:
  ...
  given contactEncoder(using enc: Encoder[List[Person]]): Encoder[Contacts] =
